...
void solve_bieq_aqd1(AQD1 *qd)
{
  void initalize_cmd(CMD *cm);
  void finalize_cmd(CMD *cm);
  void create_cmatrix(CMD *cm,AQD1 *qd);
  void create_tmatrix_csr(CMD *cm,AQD1 *qd);
  void solve_tmatrix_csr(CMD *cm,AQD1 *qd);
  void solve_pv_bv(CMD *cm,AQD1 *qd);
  void solve_dpv_bv(CMD *cm,AQD1 *qd);
  void q_solve_pv_bv(CMD *cm,AQD1 *qd);
...
void initalize_cmd(CMD *cm)
{
  ...
  MN=cm->MN;
  cm->tgfn=(char **)m_alloc2(MN+1,sizeof(char *),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tgfn");
  cm->thfn=(char **)m_alloc2(MN+1,sizeof(char *),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->thfn");
  cm->tdgfn=(char **)m_alloc2(MN+1,sizeof(char *),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tdgfn");
  cm->tdhfn=(char **)m_alloc2(MN+1,sizeof(char *),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tdhfn");
  cm->tdffn=(char **)m_alloc2(MN+1,sizeof(char *),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tdffn");
  for(i=0;i<=MN;i++){
    cm->tgfn[i]=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tgfn[i]");
    cm->thfn[i]=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->thfn[i]");
    sprintf(cm->tgfn[i],"tmpG_%05d.dat",i);
    sprintf(cm->thfn[i],"tmpH_%05d.dat",i);
    cm->tdgfn[i]=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tdgfn[i]");
    cm->tdhfn[i]=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tdhfn[i]");
    cm->tdffn[i]=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->tdffn[i]");
    sprintf(cm->tdgfn[i],"tmpdG_%05d.dat",i);
    sprintf(cm->tdhfn[i],"tmpdH_%05d.dat",i);
    sprintf(cm->tdffn[i],"tmpdF_%05d.dat",i);
  }

  cm->aval=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->aval");
  cm->aptr=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->aptr");
  cm->aidx=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->aidx");
  sprintf(cm->aval,"tmpAval.dat");
  sprintf(cm->aptr,"tmpAprt.dat");
  sprintf(cm->aidx,"tmpAidx.dat");
  cm->b=(char *)m_alloc2(16,sizeof(char ),"bem3_aw_qd1_solve_bieq.c, initialize_cmd(), cm->b");
...
void create_cmatrix(CMD *cm,AQD1 *qd)
{
  void create_cmatrix_domain(int did,CMD *cm,AQD1 *qd);
...
void create_cmatrix_domain(int did,CMD *cm,AQD1 *qd)
{
  ...
  tG=(double complex *)m_alloc2(N*N,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),tG"); // malloc
  tH=(double complex *)m_alloc2(N*N,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),tH"); // malloc
  if((fg=fopen(cm->tgfn[did],"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),*fg. Failed to create %s file.\n",cm->tgfn[did]);    exit(1);  }
  if((fh=fopen(cm->thfn[did],"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),*fh. Failed to create %s file.\n",cm->thfn[did]);    exit(1);  }

  tdG=(double complex *)m_alloc2(2*N*N,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),tdG"); // malloc
  tdH=(double complex *)m_alloc2(2*N*N,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),tdH"); // malloc
  tdF=(double *)m_alloc2(3*N,sizeof(double),"bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),tdF"); // malloc
  if((fdg=fopen(cm->tdgfn[did],"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),*fdg. Failed to create %s file.\n",cm->tdgfn[did]);    exit(1);  }
  if((fdh=fopen(cm->tdhfn[did],"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),*fdh. Failed to create %s file.\n",cm->tdhfn[did]);    exit(1);  }
  if((fdf=fopen(cm->tdffn[did],"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_cmatrix_domain(),*fdf. Failed to create %s file.\n",cm->tdffn[did]);    exit(1);  }
...
void create_tmatrix_csr(CMD *cm,AQD1 *qd)
{
  void create_matrix_csr_dac(int did,FILE *av,FILE *ap,FILE *ai,FILE *b,CMD *cm,AQD1 *qd);

  FILE *av,*ai,*ap,*b;

  size_t did;

  if((av=fopen(cm->aval,"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_tmatrix_csr(),*av. Failed to create %s file.\n",cm->aval);    exit(1);  }
  if((ai=fopen(cm->aidx,"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_tmatrix_csr(),*ai. Failed to create %s file.\n",cm->aidx);    exit(1);  }
  if((ap=fopen(cm->aptr,"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_tmatrix_csr(),*ap. Failed to create %s file.\n",cm->aidx);    exit(1);  }
  if((b=fopen(cm->b,"wb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_tmatrix_csr(),*b. Failed to create %s file.\n",cm->b);    exit(1);  }
...
void create_matrix_csr_dac(int did,FILE *av,FILE *ap,FILE *ai,FILE *b,CMD *cm,AQD1 *qd)
{
  ...
  if((fg=fopen(cm->tgfn[did],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(),*fg. Failed to open %s file.\n",cm->tgfn[did]);    exit(1);  }
  if((fh=fopen(cm->thfn[did],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(),*fh. Failed to open %s file.\n",cm->thfn[did]);    exit(1);  }

  Ne=(size_t)qd->bd.sb[did].Ne;
  tG=(double complex *)m_alloc2(Ne*4,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(),tG"); // malloc
  tH=(double complex *)m_alloc2(Ne*4,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(),tH"); // malloc

  tA=(double complex *)m_alloc2(cm->na,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(),tA"); // malloc
  ti=(size_t *)m_alloc2(cm->na,sizeof(size_t),"bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(),ti"); // malloc
  ...
  for(t=1;t<=Ne;t++){
    ...
    for(tn=0;tn<4;tn++){
      if(fread(tG,sizeof(double complex),Ne*4,fg)!=Ne*4){
        printf("bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(), failed to read the tG. exit...\n");
        exit(1);
      }
      if(fread(tH,sizeof(double complex),Ne*4,fh)!=Ne*4){
        printf("bem3_aw_qd1_solve_bieq.c, create_matrix_csr_dac(), failed to read the tH. exit...\n");
        exit(1);
      }
...
void solve_tmatrix_csr(CMD *cm,AQD1 *qd)
{
  ...
  void tmatrix_bd_store(double complex *X,AQD1 *ad);

  int err;
  double complex *x; // results
  x=(double complex *)m_alloc2(cm->na,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, solve_tmatrix_csr(),x");
  ...
  if(err!=0){
    printf("bem3_aw_qd1_solve_bieq.c, solve_tmatrix_csr(), b_mkl_solver_pardiso() returned error. err=%d. Exit...\n",err);
    exit(1);
  }
...
void tmatrix_bd_store(double complex *X,AQD1 *qd)
{
...
void solve_pv_bv(CMD *cm,AQD1 *qd)
{
  ...
  for(d=0;d<=qd->MN;d++){
    if((fdg=fopen(cm->tdgfn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, solve_pv_bv(),*fdg. Failed to open %s file.\n",cm->tdgfn[d]);    exit(1);  }
    if((fdh=fopen(cm->tdhfn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, solve_pv_bv(),*fdh. Failed to open %s file.\n",cm->tdhfn[d]);    exit(1);  }
    if((fdf=fopen(cm->tdffn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, solve_pv_bv(),*fdf. Failed to open %s file.\n",cm->tdffn[d]);    exit(1);  }

    Ne=qd->bd.sb[d].Ne;
    tdG=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, solve_pv_bv(),tdG"); // malloc
    tdH=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, solve_pv_bv(),tdH"); // malloc
    ...
    for(t=1;t<=Ne;t++){
      ...
      for(tn=0;tn<4;tn++){
        if(fread(tdG,sizeof(double complex),2*Ne*4,fdg)!=2*Ne*4){
          printf("bem3_aw_qd1_solve_bieq.c, solve_pv_bv(), failed to read the tdG. exit...\n");
          exit(1);
        }
        if(fread(tdH,sizeof(double complex),2*Ne*4,fdh)!=2*Ne*4){
          printf("bem3_aw_qd1_solve_bieq.c, solve_pv_bv(), failed to read the tdH. exit...\n");
          exit(1);
        }
        if(fread(tdF,sizeof(double),3,fdf)!=3){
          printf("bem3_aw_qd1_solve_bieq.c, solve_pv_bv(), failed to read the tdF. exit...\n");
          exit(1);
        }
...
void solve_dpv_bv(CMD *cm,AQD1 *qd)
{
  ...
  for(d=0;d<=qd->MN;d++){
    ...
    if((fg=fopen(cm->tgfn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),*fg. Failed to open %s file.\n",cm->tgfn[d]);    exit(1);  }
    if((fh=fopen(cm->thfn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),*fh. Failed to open %s file.\n",cm->thfn[d]);    exit(1);  }
    ...
    A =(MKL_Complex16 *)m_alloc2(nn*lda,sizeof(MKL_Complex16),"bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),A"); // malloc
    B =(MKL_Complex16 *)m_alloc2(nn*ldb,sizeof(MKL_Complex16),"bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),B"); // malloc
    ipiv=(MKL_INT *)m_alloc2(nn,sizeof(MKL_INT),"bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),ipiv"); // malloc
    tG=(double complex *)m_alloc2(4*Ne,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),tG"); // malloc
    tH=(double complex *)m_alloc2(4*Ne,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(),tH"); // malloc
    ...
    for(t=1;t<=Ne;t++){
      ...
      for(tn=0;tn<4;tn++){
        if(fread(tG,sizeof(double complex),Ne*4,fg)!=Ne*4){
          printf("bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(), failed to read the tG. exit...\n");
          exit(1);
        }
        if(fread(tH,sizeof(double complex),Ne*4,fh)!=Ne*4){
          printf("bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(), failed to read the tH. exit...\n");
          exit(1);
        }
        if( tn==3 && ELT3==check_element_type(atd,&(qd->bd)) )  continue;
    ...
    if(info!=0){
      printf("bem3_aw_qd1_solve_bieq.c, solve_dpv_bv(), LAPACKE_zgesv(). info=%lld error. Exit...\n",info);
      exit(1);
    }
...
void q_solve_pv_bv(CMD *cm,AQD1 *qd)
{
  ...
  tpv=(double complex *)m_alloc2(qd->bd.NN*3,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(),tpv"); // malloc

  for(d=1;d<=qd->MN;d++){
    if((fdg=fopen(cm->tdgfn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(),*fdg. Failed to open %s file.\n",cm->tdgfn[d]);    exit(1);  }
    if((fdh=fopen(cm->tdhfn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(),*fdh. Failed to open %s file.\n",cm->tdhfn[d]);    exit(1);  }
    if((fdf=fopen(cm->tdffn[d],"rb"))==NULL){    printf("bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(),*fdf. Failed to open %s file.\n",cm->tdffn[d]);    exit(1);  }

    Ne=qd->bd.sb[d].Ne;
    tdG=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(),tdG"); // malloc
    tdH=(double complex *)m_alloc2(2*Ne*4,sizeof(double complex),"bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(),tdH"); // malloc
    ...
    for(t=1;t<=Ne;t++){
      ...
      for(tn=0;tn<4;tn++){
        if(fread(tdG,sizeof(double complex),2*Ne*4,fdg)!=2*Ne*4){
          printf("bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(), failed to read the tdG. exit...\n");
          exit(1);
        }
        if(fread(tdH,sizeof(double complex),2*Ne*4,fdh)!=2*Ne*4){
          printf("bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(), failed to read the tdH. exit...\n");
          exit(1);
        }
        if(fread(tdF,sizeof(double),3,fdf)!=3){
          printf("bem3_aw_qd1_solve_bieq.c, q_solve_pv_bv(), failed to read the tdF. exit...\n");
          exit(1);
        }
        if( tn==3 && ELT3==check_element_type(atd,&(qd->bd)) )  continue;
