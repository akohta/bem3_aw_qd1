...
{
  void bil_force_4p(double *F,double *N,double *rc,int s,AQD1 *ad);
  void bil_force_9p(double *F,double *N,double *rc,int s,AQD1 *ad);
  void lit_force_4p(double *F,double *N,double *rc,int s,AQD1 *ad);
  void lit_force_7p(double *F,double *N,double *rc,int s,AQD1 *ad);
...
{
  void bil_sa_4p(double *S,int d,int s,AQD1 *ad);
  void bil_sa_9p(double *S,int d,int s,AQD1 *ad);
  void lit_sa_4p(double *S,int d,int s,AQD1 *ad);
  void lit_sa_7p(double *S,int d,int s,AQD1 *ad);
...
void bil_force_4p(double *F,double *N,double *rc,int s,AQD1 *ad)
{
  ...
  c1=0.25/(ad->pw.c0*ad->pw.c0*ad->pw.rho0); // 1/(4*K0)
  c2=0.25*ad->pw.rho0; // rho0/4
  c3=0.50*ad->pw.rho0; // rho0/2
  ...
  for(i=0;i<4;i++){
    ...
    calc_apw_pv(&p,v,r,&(ad->pw)); // aw_pw.h
...
void bil_force_9p(double *F,double *N,double *rc,int s,AQD1 *ad)
{
  ...
  for(i=0;i<9;i++){
    ...
    pv_t_bd_aqd1(&p,v,0,s,ad->bd.zt_49[i],ad->bd.et_49[i],1,ad);
...
void lit_force_4p(double *F,double *N,double *rc,int s,AQD1 *ad)
{
  ...
  for(i=0;i<4;i++){
    if(i<3){
      ...
      pv_t_bd_aqd1(&p,v,0,s,ad->bd.zt_34[i],ad->bd.et_34[i],0,ad);
    }
...
void lit_force_7p(double *F,double *N,double *rc,int s,AQD1 *ad)
{
  ...
  for(i=0;i<7;i++){
    ...
    pv_t_bd_aqd1(&p,v,0,s,ad->bd.zt_37[i],ad->bd.et_37[i],1,ad);
