...
void read_aqd1(int argc,char **argv,AQD1 *qd)
{
  void filename_chk(int argc,char **argv);
  void read_periodicity_data(char *ped_fn,AQD1 *qd);
  void read_medium_data(char *med_fn,AQD1 *qd);
  void read_mesh_data(char *msh_fn,AQD1 *qd);
...
void print_aqd1(AQD1 *qd)
{
  void print_periodicity_data(AQD1 *qd);
  void print_medium_data(AQD1 *qd);
  void print_mesh_data(AQD1 *qd);
...
void initialize_aqd1(AQD1 *qd)
{
  void rotation_translation_obj(double *rv,double th,double *tv,AQD1 *ad);
  void init_elem_const(BOUD *bd);
  void malloc_sub_domain(AQD1 *ad);
  void init_sub_domain(AQD1 *ad);
  void init_boundary_data(AQD1 *ad);
  ...
  qd->bd.qd.kx=qd->pw.wd.kx;
  if(qd->bd.ps!=0){
    if(qd->bd.qd.kx==0.0){ // kx==0.0
      printf("kx must not be equal to 0 (must satisfy quasi-peridic boundary condition). Exit...\n");
      exit(1);
    }
    if(qd->bd.qd.kx==qd->bd.qd.k){ // kx=k
      printf("kx must not be equal to k (The wave source and the objcet must not overlap). Exit...\n");
    }
  }
  // medium
...
void finalize_aqd1(AQD1 *qd)
{
  void mfree_sub_domain(AQD1 *qd);
...
int domain_id_aqd1(double *ro,AQD1 *qd)
{
  int domain_id(double *rt,AQD1 *qd);
  double fid_calc_solid_angle(int type,double r[4][3],int *flg);

  double rv[4][3],omega,rt[3];
  double *Og=(double *)m_alloc2(qd->MN+1,sizeof(double),"bem3_aw_qd1.c, domain_id_aqd1()");
  int i,j,k,d,flg,l;

  if(qd->bd.ps==0) return domain_id(ro,qd);

  l=domain_id_l_aqd1(ro,qd);
  rt[0]=ro[0]-(double)l*qd->bd.qd.d;
  rt[1]=ro[1];
  rt[2]=ro[2];
...
int domain_id_l_aqd1(double *rt,AQD1 *qd)
{
  return (int)floor(rt[0]/qd->bd.qd.d+0.5);
}

void dat_write_aqd1(char *fname,AQD1 *qd)
{
  FILE *fp;
  int i,j,d;
...
void dat_read_aqd1(char *fname,AQD1 *qd)
{
  void malloc_node(BOUD *bd);  
  void malloc_elem(BOUD *bd); 
  void init_elem_const(BOUD *bd);
  void malloc_sub_domain(AQD1 *md);
  ...
  if(fread(qd,sizeof(AQD1),1,fp)!=1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the qd. exit...\n");
    exit(1);
  }
  // material def
  ...
  if(fread(qd->rho0,sizeof(double),qd->MN+1,fp)!=qd->MN+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the rho0. exit...\n");
    exit(1);
  }
  if(fread(qd->c0,sizeof(double),qd->MN+1,fp)!=qd->MN+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the c0. exit...\n");
    exit(1);
  }
  if(fread(qd->k0,sizeof(double),qd->MN+1,fp)!=qd->MN+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the k0. exit...\n");
    exit(1);
  }
  if(fread(qd->K0,sizeof(double),qd->MN+1,fp)!=qd->MN+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the K0. exit...\n");
  ...
  if(fread(qd->k1,sizeof(double complex),qd->MN+1,fp)!=qd->MN+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the k1. exit...\n");
    exit(1);
  }
  if(fread(qd->k2,sizeof(double complex),qd->MN+1,fp)!=qd->MN+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the k2. exit...\n");
    exit(1);
  }
  // incident field
  if(fread(&(qd->pw),sizeof(Apw),1,fp)!=1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the pw. exit...\n");
    exit(1);
  }
  // BOUD
  malloc_node(&(qd->bd)); // malloc
  malloc_elem(&(qd->bd)); // malloc
  for(i=0;i<=qd->bd.Nn;i++){
    if(fread(qd->bd.rn[i],sizeof(double),3,fp)!=3){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the rn[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    if(fread(qd->bd.ed[i],sizeof(int),4,fp)!=4){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the ed[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    if(fread(qd->bd.eni[i],sizeof(int),4,fp)!=4){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the eni[i]. exit...\n");
      exit(1);
    }
  }
  if(fread(qd->bd.md,sizeof(int),qd->bd.Ne+1,fp)!=qd->bd.Ne+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the md. exit...\n");
    exit(1);
  }
  if(fread(qd->bd.sd,sizeof(int),qd->bd.Ne+1,fp)!=qd->bd.Ne+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the sd. exit...\n");
    exit(1);
  }
  if(fread(qd->bd.gd,sizeof(int),qd->bd.Ne+1,fp)!=qd->bd.Ne+1){
    printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the qd. exit...\n");
    exit(1);
  }
  for(i=0;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      if(fread(qd->bd.ren[i][j],sizeof(double),3,fp)!=3){
        printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the ren[i][j]. exit...\n");
        exit(1);
      }
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    for(j=0;j<4;j++){
      if(fread(qd->bd.wen[i][j],sizeof(double),3,fp)!=3){
        printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the wen[i][j]. exit...\n");
        exit(1);
      }
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    if(fread(qd->bd. Pi[i],sizeof(double complex),4,fp)!=4){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the Pi[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=qd->bd.Ne;i++){
    if(fread(qd->bd.dPi[i],sizeof(double complex),4,fp)!=4){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the dPi[i]. exit...\n");
      exit(1);
    }
  }
  ...
  for(d=0;d<=qd->MN;d++){
    if(fread(&tmp,sizeof(int),1,fp)!=1){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the tmp. exit...\n");
      exit(1);
    }
    if(fread(qd->bd.sb[d].sid,sizeof(int),qd->bd.sb[d].Ne+1,fp)!=qd->bd.sb[d].Ne+1){
      printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the sid. exit...\n");
      exit(1);
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      if(fread(qd->bd.sb[d]. P[i],sizeof(double complex),4,fp)!=4){
        printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the P[i]. exit...\n");
        exit(1);
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      if(fread(qd->bd.sb[d].dP[i],sizeof(double complex),4,fp)!=4){
        printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the qd. exit...\n");
        exit(1);
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d]. pv[i][j],sizeof(double complex),3,fp)!=3){
          printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the pv[i][j]. exit...\n");
          exit(1);
        }
      }
    }
    for(i=0;i<=qd->bd.sb[d].Ne;i++){
      for(j=0;j<4;j++){
        if(fread(qd->bd.sb[d].dpv[i][j],sizeof(double complex),3,fp)!=3){
          printf("bem3_aw_qd1.c, dat_read_aqd1(), failed to read the dpv[i][j]. exit...\n");
          exit(1);
        }
      }
    }
...
void read_periodicity_data(char *ped_fn,AQD1 *qd)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("bem3_aw_qd1.c, read_periodicity_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("bem3_aw_qd1.c, read_periodicity_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("bem3_aw_qd1.c, read_periodicity_data(), failed to read the ps. exit...\n");
    exit(1);
  }
  qd->bd.ps=ti;
  if(fscanf(fp,"%lf",&td)!=1){
    printf("bem3_aw_qd1.c, read_periodicity_data(), failed to read the d. exit...\n");
    exit(1);
  }
  qd->bd.qd.d=td;
}
...
void read_medium_data(char *med_fn,AQD1 *qd)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("bem3_aw_qd1.c, read_medium_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("bem3_aw_qd1.c, read_medium_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("bem3_aw_qd1.c, read_medium_data(), failed to read the MN. exit...\n");
    exit(1);
  }
  qd->MN=ti;

  // malloc
  qd->rho0=(double *)m_alloc2(qd->MN+1,sizeof(double),"memory allocation error! setup.c,read_medium_data(),qd->rho0");
  qd->c0  =(double *)m_alloc2(qd->MN+1,sizeof(double),"memory allocation error! setup.c,read_medium_data(),qd->c0");
  qd->k0  =(double *)m_alloc2(qd->MN+1,sizeof(double),"memory allocation error! setup.c,read_medium_data(),qd->k0");
  qd->K0  =(double *)m_alloc2(qd->MN+1,sizeof(double),"memory allocation error! setup.c,read_medium_data(),qd->K0");
  qd->k1=(double complex *)m_alloc2(qd->MN+1,sizeof(double complex),"memory allocation error! setup.c,read_medium_data(),qd->k1");
  qd->k2=(double complex *)m_alloc2(qd->MN+1,sizeof(double complex),"memory allocation error! setup.c,read_medium_data(),qd->k2");
  ...
  for(i=1;i<=qd->MN;i++){
    if(fscanf(fp,"%lf",&td)!=1){
      printf("bem3_aw_qd1.c, read_medium_data(), failed to read the rho0. exit...\n");
      exit(1);
    }
    qd->rho0[i]=td;
    if(fscanf(fp,"%lf",&td)!=1){
      printf("bem3_aw_qd1.c, read_medium_data(), failed to read the c0. exit...\n");
      exit(1);
    }
    qd->c0[i]=td;
  }
...
void read_mesh_data(char *msh_fn,AQD1 *md)
{
    void malloc_node(BOUD *bd);
  void malloc_elem(BOUD *bd);
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fscanf(fp,"%lf",&td)!=1){
    printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the msh version. exit...\n");
    exit(1);
  }
  // check file version
  if(td<MSHVER){
  ...
  if(fscanf(fp,"%d",&ti)!=1){
    printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the msh formant. exit...\n");
    exit(1);
  }
  //check data format
  if(ti!=MSHASCI){
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the data precision. exit...\n");
    exit(1);
  }
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the Nn. exit...\n");
    exit(1);
  } 
  md->bd.Nn=ti;
  malloc_node(&(md->bd));
  for(i=1;i<=md->bd.Nn;i++){
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the id. exit...\n");
      exit(1);
    }
    if(ti!=i)       printf("bad id %d\n",ti);
    if(fscanf(fp,"%lf",&td)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the rn[i][0]. exit...\n");
      exit(1);
    }
    md->bd.rn[i][0]=td;
    if(fscanf(fp,"%lf",&td)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the rn[i][1]. exit...\n");
      exit(1);
    }
    md->bd.rn[i][1]=td;
    if(fscanf(fp,"%lf\n",&td)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the rn[i][2]. exit...\n");
      exit(1);
    }
    md->bd.rn[i][2]=td;
  }
  ...
  if(fscanf(fp,"%d",&ti)!=1){
    printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the Ne. exit...\n");
    exit(1);
  }
  md->bd.Ne=ti/2; 
  malloc_elem(&(md->bd));
  ...
  for(i=1;i<=md->bd.Ne;i++){
    ...
    // element type
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the type. exit...\n");
      exit(1);
    }
    etype=ti;
    if(ti!=ELT3 && ti!=ELT4){
    ...
    // number of tags
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the number of tags. exit...\n");
      exit(1);
    }
    for(j=0;j<ti;j++){
      if(fscanf(fp,"%d",&ti2)!=1){
        printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the physical entity. exit...\n");
        exit(1);
      }
    ...
    // node id
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the ed[i][0]. exit...\n");
      exit(1);
    }
    md->bd.ed[i][0]=ti;
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the ed[i][1]. exit...\n");
      exit(1);
    }
    md->bd.ed[i][1]=ti;
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the ed[i][2]. exit...\n");
      exit(1);
    }
    md->bd.ed[i][2]=ti;
    if(etype==ELT3) md->bd.ed[i][3]=0;
      ...
      if(fscanf(fp,"%d",&ti)!=1){
        printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the ed[i][3]. exit...\n");
        exit(1);
      }
      md->bd.ed[i][3]=ti;
    }
    ...
    // element id
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the element id. exit...\n");
      exit(1);
    }
    if(ti!=i*2){
      printf("bad id :%d. Exit...\n",ti);
    ...
    // element type
    if(fscanf(fp,"%d",&ti)!=1){
      printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the element type. exit...\n");
      exit(1);
    }
    etype=ti;
    if(ti!=ELT3 && ti!=ELT4){
    ...
    for(j=0;j<ti;j++){
      if(fscanf(fp,"%d",&ti2)!=1){
        printf("bem3_aw_qd1.c, read_mesh_data(), failed to read the domain id. exit...\n");
        exit(1);
      }
      if(j==0){ // domain id
        if(ti2==OPENDID) ti2=0;
...
void malloc_elem(BOUD *bd)
{
  ...
  bd-> Pi=(double complex **)m_alloc2(Ne+1,sizeof(double complex *),"setup.c, malloc_elem(), bd->Pi");
  bd->dPi=(double complex **)m_alloc2(Ne+1,sizeof(double complex *),"setup.c, malloc_elem(), bd->dPi");
  for(i=0;i<=Ne;i++){
    ...
    bd-> Pi[i]=(double complex *)m_alloc2(4,sizeof(double complex),"setup.c, malloc_elem(), bd->Pi[i]");
    bd->dPi[i]=(double complex *)m_alloc2(4,sizeof(double complex),"setup.c, malloc_elem(), bd->dPi[i]");
    for(j=0;j<4;j++){
...
void malloc_sub_domain(AQD1 *qd)
{
  int *Nc,i,j,k;
  Nc=(int *)m_alloc2(qd->MN+1,sizeof(int),"setup.c, malloc_sub_domain(), Nc");
  ...
  qd->bd.sb=(SUBD *)m_alloc2(qd->MN+1,sizeof(SUBD),"setup.c, malloc_sub_domain(), ad->bd.sb");
  for(i=0;i<=qd->MN;i++){
    qd->bd.sb[i].Ne=Nc[i];
    qd->bd.sb[i].sid=(int *)m_alloc2(Nc[i]+1,sizeof(int),"setup.c, malloc_sub_domain(), ad->bd.sb[i],sid");
    qd->bd.sb[i].P =(double complex **)m_alloc2(Nc[i]+1,sizeof(double complex *),"setup.c, malloc_sub_domain(), qd->bd.sb[i].P");
    qd->bd.sb[i].dP=(double complex **)m_alloc2(Nc[i]+1,sizeof(double complex *),"setup.c, malloc_sub_domain(), qd->bd.sb[i].dP");
    qd->bd.sb[i].pv =(double complex ***)m_alloc2(Nc[i]+1,sizeof(double complex **),"setup.c, malloc_sub_domain(), qd->bd.sb[i].pv");
    qd->bd.sb[i].dpv=(double complex ***)m_alloc2(Nc[i]+1,sizeof(double complex **),"setup.c, malloc_sub_domain(), qd->bd.sb[i].dpv");
    for(j=0;j<=Nc[i];j++){
      qd->bd.sb[i].P [j]=(double complex *)m_alloc2(4,sizeof(double complex),"setup.c, malloc_sub_domain(), qd->bd.sb[i].P[j]");
      qd->bd.sb[i].dP[j]=(double complex *)m_alloc2(4,sizeof(double complex),"setup.c, malloc_sub_domain(), qd->bd.sb[i].dP[j]");
      qd->bd.sb[i].pv [j]=(double complex **)m_alloc2(4,sizeof(double complex),"setup.c, malloc_sub_domain(), qd->bd.sb[i].pv[j]");
      qd->bd.sb[i].dpv[j]=(double complex **)m_alloc2(4,sizeof(double complex),"setup.c, malloc_sub_domain(), qd->bd.sb[i].dpv[j]");
      for(k=0;k<4;k++){
        qd->bd.sb[i].pv [j][k]=(double complex *)m_alloc2(3,sizeof(double complex),"setup.c, malloc_sub_domain(), qd->bd.sb[i].pv[j][k]");
        qd->bd.sb[i].dpv[j][k]=(double complex *)m_alloc2(3,sizeof(double complex),"setup.c, malloc_sub_domain(), qd->bd.sb[i].dpv[j][k]");
      }
    }
  }
...
void init_boundary_data(AQD1 *qd)
{
