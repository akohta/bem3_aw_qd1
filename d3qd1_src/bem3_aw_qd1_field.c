...
{
  int domain_id(double *rt,AQD1 *qd);// bem3_aw_qd1.c, non-periodicity
  ...
  if(ad->bd.ps==1){
    l=domain_id_l_aqd1(rt,ad);
    r0[0]=rt[0]-(double)l*ad->bd.qd.d;
    r0[1]=rt[1];
    r0[2]=rt[2];
    did=domain_id(r0,ad);
  }
  ...
  *phi=0.0;
  ...
  if(l!=0){
		arg=(double)l*ad->bd.qd.kx*ad->bd.qd.d;
...
{
  int pv_s_bieq_aqd1(double complex *p,double complex *pv,double *rt,int type,AQD1 *ad);
  int pv_s_dbieq_aqd1(double complex *p,double complex *pv,double *rt,int type,AQD1 *ad);
  
  int did;

  did=pv_s_dbieq_aqd1(p,pv,rt,type,ad);
  if(did<0){ // near boundary
  	did=pv_s_bieq_aqd1(p,pv,rt,type,ad);
  }
...
int pv_t_bieq_aqd1(double complex *p,double complex *pv,double *rt,int type,AQD1 *ad)
{
...
int pv_i_bieq_aqd1(double complex *p,double complex *pv,double *rt,int type,AQD1 *ad)
{
  int did;

  did=domain_id_aqd1(rt,ad);
...
int pv_t_dbieq_aqd1(double complex *p,double complex *pv,double *rt,int type,AQD1 *ad)
{
  double complex pi,pvi[3];
  int did,l;
...
int pv_i_dbieq_aqd1(double complex *p,double complex *pv,double *rt,int type,AQD1 *ad)
{
  return pv_i_aqd1(p,pv,rt,type,ad);
}
