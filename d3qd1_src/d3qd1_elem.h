...
#ifndef D3QD1_ELEM_H_
#define D3QD1_ELEM_H_
...
typedef struct sub_domain_data{
  ...
  double complex **P,**dP;    // velocity potential boundary value, P[element id][element node id(0~3)]
  double complex ***pv,***dpv;// particle velocity boundary value, pv[element id][element node id][dimension id(0:pvx,1:pvy,2:pvz)]
}SUBD;

typedef struct boundary_data{
  // periodicity data
  int ps;  // 1 : quasi-periodic boundary condition, 0 : non periodic
  QPDT qd; // data for quasi-periodic green's function
  // gmsh data
  ...
  double complex **Pi,**dPi; // incident field data on boundary, Pi[element id][element node id]
...
typedef struct domain_data {
  ...
  double *rho0,*c0;       // dencity of the medium, wave speed in the medium
  
  double *k0,*K0;         // wave number of medium, bulk modulus(K0=rho0*c0*c0)
  double complex *k1,*k2; // wave constant (k1=I*omega/K0, k2=I*omega*rho0)

  Apw pw; // incident field data, aw_pw.h
...
typedef struct coef_matrix{
  ...
  // derivative coefficient datafile name
  char **tdgfn;
  char **tdhfn;
  char **tdffn;
