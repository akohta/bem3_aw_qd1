...
void p_field_x(IMD *id,AQD1 *ad);
void p_field_y(IMD *id,AQD1 *ad);
void p_field_z(IMD *id,AQD1 *ad);
void output_field(char *pl,IMD *id,AQD1 *ad);
...
int main(int argc,char *argv[])
{
  ...
  id.rang=3.0*ad.pw.wd.lambda0;           // range of sampling
  id.ts=40;                            // time step per cycle
  id.type=0;                           // type setting for total_field_amsp()
...
void p_field_x(IMD *id,AQD1 *ad)
{
  ...
  for(i=0;i<id->m;i++){
    ...
    for(j=0;j<id->m;j++){
      ...
      pv_t_aqd1(&p,v,x,id->type,ad); // total field
...
void output_field(char *pl,IMD *id,AQD1 *ad)
{
  ...
  dt=1.0/(ad->pw.f*(double)id->ts);
  
  #pragma omp parallel for schedule(dynamic) 
  for(n=0;n<id->ts;n++){
    output_png(n,cexp(-I*2.0*M_PI*ad->pw.f*dt*(double)n),pl,id);
  }
